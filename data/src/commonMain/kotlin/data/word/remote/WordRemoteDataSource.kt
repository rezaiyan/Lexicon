...
class WordRemoteDataSource(
    ...
    suspend fun getWords(): Result<List<RemoteWord>> =
        apiClient.get<List<RemoteWord>>("/words")
            .map { it ?: emptyList() }

    suspend fun upsertWords(words: List<RemoteWord>): Result<Unit> =
        apiClient.postUnit(
            path = "/words",
            body = UpsertWordsPayload(words)
        )

    suspend fun updateWord(id: Long, word: RemoteWord): Result<Unit> =
        apiClient.patchUnit(
            path = "/words/$id",
            body = word
        )
    ...
    fun deleteWords(ids: List<Long>): Flow<Result<Unit>> =
        if (ids.isEmpty()) {
