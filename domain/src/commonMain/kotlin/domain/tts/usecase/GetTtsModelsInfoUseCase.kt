 ...
 * including download status, on-disk size, and the user's selected voice per model.
 */
class GetTtsModelsInfoUseCase(
    ...
    suspend operator fun invoke(): Try<List<TtsModelInfo>> = Try {
        ...
        supportedCodes.map { code ->
            ...
            val modelInfo = ttsRepository.getModelInfo(code, language.displayName).getOrThrow()
            val selectedSpeakerId = settingsRepository.getTtsVoiceForLanguage(code).first()
            modelInfo.copy(selectedSpeakerId = selectedSpeakerId)
